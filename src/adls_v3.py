...
class DirectoryClient:
  ...
  def upload_str(self,str_content,dest):
    '''
    Upload a python str to a path inside the container
    '''
    print(f'Uploading {str_content} to {dest}')
    self.client.upload_blob(name=dest,data=str_content,overwrite=True)
  ...
  def download_in_py(self, source):
    '''
    Download a single file to a python memory
    '''
    print(f'Downloading {source}.')
    bc = self.client.get_blob_client(blob=source)
    data=str(bc.download_blob().readall(),"utf-8")
    return data
